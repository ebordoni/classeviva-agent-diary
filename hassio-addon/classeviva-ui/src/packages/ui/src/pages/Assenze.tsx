    ...
    <div>
      ...
      {!isLoading && totale > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          ...
          <StatCard label="Da giustificare" value={nonGiustificate} color="orange" />
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs uppercase">
                ...
                <th className="px-4 py-2 text-left hidden md:table-cell">Motivazione</th>
              </tr>
            </thead>
            <tbody>
              {events.map((e, i) => (
                <tr key={`${e.evtDate}-${e.evtCode}-${i}`} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  <td className="px-4 py-2 text-gray-700">
                    {format(parseISO(e.evtDate), "EEE d MMM yyyy", { locale: it })}
                  </td>
                  <td className="px-4 py-2">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${evtBadge(e.evtCode)}`}>
                      {evtLabel(e.evtCode)}
                    </span>
                  </td>
                  <td className="px-4 py-2">
